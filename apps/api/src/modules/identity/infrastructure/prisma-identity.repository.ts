...
export class PrismaIdentityRepository implements IdentityRepository {
  ...
  async findWebCredential(loginUsername: string): Promise<WebCredentialRecord | null> {
  ...
  private async uniqueTelegramUsername(
    ...
    let candidate = base;
    let suffix = 0;
    while (await tx.userPresentation.findUnique({ where: { username: candidate }, select: { userId: true } })) {
      suffix += 1;
