...
export class GamerMatchService {
  ...
  async setRoomCode(userId: string, gameId: string, challengeId: string, roomCode: string) {
    ...
    if (access.challengerUserId !== userId) {
      throw new AppError(403, "GAMER_MATCH_HOST_REQUIRED", "Only the challenger can publish the EA FC room code");
    }
    ...
    if (!updated) {
      throw new AppError(409, "GAMER_MATCH_STATE_CHANGED", "Match state changed; refresh and try again");
    }
    return updated;
  }
  ...
  async submitResult(
    ...
    if (!input.proof.byteLength || input.proof.byteLength > RESULT_PROOF_MAX_BYTES) {
      throw new AppError(413, "GAMER_MATCH_PROOF_TOO_LARGE", "Match proof must be between 1 byte and 5 MB");
    }
    if (!this.storage) {
      throw new AppError(
        503,
    ...
    const match = await this.matches.ensureForAcceptedChallenge(challengeId);
    if (!match.roomCode) {
      throw new AppError(409, "GAMER_MATCH_ROOM_CODE_REQUIRED", "The EA FC room code must be published before results can be submitted");
    }
    ...
    if (!updated) {
      throw new AppError(409, "GAMER_MATCH_STATE_CHANGED", "Match state changed while the result was submitted");
    }
    return updated;
  }
  ...
  async resolveDispute(adminUserId: string, matchId: string, resolution: GamerDisputeResolutionInput) {
    await this.platformAdmin.requirePlatformAdmin(adminUserId);
    const updated = await this.matches.resolveDispute({ matchId, actorUserId: adminUserId, resolution });
    if (!updated) throw new AppError(409, "GAMER_MATCH_NOT_DISPUTED", "Match is no longer disputed");
    return updated;
  }
  ...
  async getDisputeProof(adminUserId: string, matchId: string, side: GamerMatchSide) {
    await this.platformAdmin.requirePlatformAdmin(adminUserId);
    if (!this.storage) {
      throw new AppError(503, "OBJECT_STORAGE_NOT_CONFIGURED", "Match proof storage is not configured");
    }
    const submission = await this.matches.getSubmissionProof(matchId, side);
    if (!submission) throw new AppError(404, "GAMER_MATCH_PROOF_NOT_FOUND", "Match proof not found");
  ...
  private async requireAcceptedEaFcChallenge(userId: string, gameId: string, challengeId: string) {
    ...
    if (!game || game.slug !== EA_FC_MOBILE_SLUG) {
      throw new AppError(404, "EA_FC_MATCH_BRIDGE_NOT_AVAILABLE", "EA SPORTS FC Mobile match bridge is not available for this game");
    }
    ...
    if (access.challengerUserId !== userId && access.challengedUserId !== userId) {
      throw new AppError(403, "GAMER_MATCH_FORBIDDEN", "Only match participants can access this EA FC match");
    }
    if (access.record.status !== "ACCEPTED") {
      throw new AppError(409, "GAMER_MATCH_REQUIRES_ACCEPTED_CHALLENGE", "The challenge must be accepted before creating an EA FC match");
    }
    return access;
  }
}
