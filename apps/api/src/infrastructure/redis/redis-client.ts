...
export class RedisClientError extends Error {
  constructor(
    readonly kind: RedisClientErrorKind,
    message: string,
  ) {
    super(message);
...
type PendingCommand = {
  ...
  reject: (error: RedisClientError) => void;
...
function parseResp(buffer: Buffer, offset = 0): { value: RedisValue; offset: number } | null {
  ...
  if (prefix === "-") {
    throw new RedisClientError("COMMAND", "Redis rejected the command");
  }
  if (prefix === "$") {
    const length = Number(line);
    if (length === -1) return { value: null, offset: next };
    if (!Number.isInteger(length) || length < 0) {
      throw new RedisClientError("PROTOCOL", "Invalid Redis bulk response");
    }
  ...
  if (prefix === "*") {
    ...
    if (!Number.isInteger(count) || count < 0) {
      throw new RedisClientError("PROTOCOL", "Invalid Redis array response");
    }
  ...
  throw new RedisClientError("PROTOCOL", "Unsupported Redis response");
}

function unavailableError(): RedisClientError {
  return new RedisClientError("UNAVAILABLE", "Redis is unavailable");
}
...
export class RedisClient {
  ...
  async command(parts: readonly string[]): Promise<RedisValue> {
    if (parts.length === 0) {
      throw new RedisClientError("PROTOCOL", "Redis command cannot be empty");
    }
  ...
  async ping(): Promise<void> {
    ...
    if (result !== "PONG") {
      throw new RedisClientError("PROTOCOL", "Redis PING returned an invalid response");
    }
  }
  ...
  private openSocket(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const connectTimeout = setTimeout(() => {
        if (settled) return;
        settled = true;
        const error = new RedisClientError("TIMEOUT", "Redis connection timed out");
  ...
  private onData(chunk: Buffer): void {
    ...
    try {
      ...
      this.failConnection(
        error instanceof RedisClientError
          ? error
          : new RedisClientError("PROTOCOL", "Invalid Redis response"),
      );
    }
  }

  private failConnection(error: RedisClientError): void {
    const socket = this.socket;
    this.socket = null;
    this.ready = false;
    this.responseBuffer = Buffer.alloc(0);
  ...
  private rejectPending(error: RedisClientError): void {
