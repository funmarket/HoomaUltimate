...
import { teamApi, type ManagedTeam, type TeamControlDetail } from "../api/team-client";
...
export function TeamDetailPage({ teamId }: { readonly teamId: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    void teamApi.publicDetail(teamId).then((response) => {
    ...
    void teamApi.managed().then((response) => {
    ...
    <div className="page team-profile-page">
      ...
      <div className="two-col team-profile-grid">
        ...
        <section className="panel">
          ...
          <div className="list">
            {team.responsibilities.length ? team.responsibilities.map((responsibility) => (
              <div className="list-row" key={`${responsibility.userId}-${responsibility.role}`}>
                <div className="team-player-identity">
                  ...
                  <small>{responsibility.user.presentation?.username ? `@${responsibility.user.presentation.username}` : "HOOMA member"}</small>
                </div>
          ...
          {team.community ? <div className="team-community-note"><span>HOOMA Community</span><strong>{team.community.name}</strong></div> : null}
        </section>
      </div>
      ...
      <section className="panel team-public-boundary">
        ...
        <p className="muted">Challenge creation and Team management remain protected member actions. Server-side Team capability rules decide who can act.</p>
        <a className="button secondary" href={canManage ? "/teams/control" : "/teams"}>{canManage ? "Open Team controls" : "Browse opponents"}</a>
      </section>
    </div>
  );
}
