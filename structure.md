...
ADR-057 keeps the private Athletes Calendar inside the Athletes domain. `AthletesCalendarEntry` is separate from the canonical Event/Play/Watch lifecycle. Active Athletes members may read bounded, cursor-paged Calendar ranges without taking the parent lifecycle row lock. Founder Calendar create/edit/cancel mutations reuse the existing Athletes community `FOR UPDATE` lifecycle lock and recheck active Founder authority inside the same transaction, so archive and Founder Calendar writes serialize through one authoritative lifecycle guard.

ADR-058 adds Athletes-owned `AthletesCalendarRsvp` state with `GOING | MAYBE | NOT_GOING`, one row per User/Calendar entry, active-member write authority, and no reuse of generic Event RSVP. ADR-059 narrows RSVP locking to a shared Athletes lifecycle guard (`FOR SHARE`) so independent RSVP writers may proceed concurrently while still conflicting with exclusive archive/member-removal/Calendar-cancellation mutations. Retained RSVP rows from former members remain durable history but are excluded from current Calendar aggregate counts.
