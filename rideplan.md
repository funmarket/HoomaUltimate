...
- Tests so far: direct unit suite passed with 184/184 tests using `npx.cmd tsx --test` and the unit-only file list; the repo `npm test` wrapper is still not usable on this Windows shell because it fails before running tests with `spawn EINVAL`.
- Static/build checks so far: `db:generate` passed; `db:validate` passed with a safe validation-only `DATABASE_URL`; full workspace `typecheck` passed; API typecheck passed after formatting; `architecture:check` passed; `build:packages` passed; Prettier was run on the two RIDE-003 source/test files.
- PostgreSQL migration/integration proof: pending. Docker is not installed on this host, so no local disposable PostgreSQL was started. The new `tests/rides.repository.integration.test.ts` is intended to run in GitHub CI against its disposable PostgreSQL before this task can be marked DONE.
