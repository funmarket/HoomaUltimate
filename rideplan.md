...
- 2026-08-31 PR #205 merged into `phase-0-foundation` with expected-head protection at `a971d3855d02e10e2c791a78fdbd3de88cfd3b6b`; merge/read-back confirmed new foundation SHA `3095b6731efbed99fdc3727d1983a7f1499d3d2c`.
- 2026-08-31 PR #205 final path now in foundation: explicit `MATCHDAY | GENERAL` Ride context, explicit `FREE | CASH` compensation in offer/request forms, human amount input, shared supported cash currency/exponent policy (`TND`, `EUR`, `USD`), deterministic integer minor-unit conversion, Ride contract enforcement, API/service/repository persistence, PostgreSQL compensation currency constraints via `packages/database/prisma/migrations/20260831235000_ride_cash_currency_policy/migration.sql`, public/owner readback, and compensation badge display.
- 2026-08-31 post-#205 overlap check: no open PRs against `phase-0-foundation`.
- 2026-08-31 remaining RIDE-007C blocker: `/rides/mine` still requires authenticated actor-owned Ride server readback for owned offers, owned requests, and passenger participations; new implementation branch `ride/rides-mine-server-readback` starts from foundation `3095b6731efbed99fdc3727d1983a7f1499d3d2c`.
- 2026-09-01 `/rides/mine` server-readback branch is `[~] IN PROGRESS`: source trace confirmed existing authenticated member Ride router under `/api/v1/rides/*`, actor identity from `getAuth(request).userId`, RideService as application composition owner, Prisma Ride repositories as Ride-owned persistence owners, existing owner indexes on `[driverUserId, status, departureAt]`, `[requesterUserId, status, createdAt]`, and `[passengerUserId, status, requestedAt]`, and no need for a new index/migration in this narrow slice.
- 2026-09-01 local implementation adds bounded `rideMineSchema` with independent `offerCursor`, `requestCursor`, and `participationCursor`, `GET /api/v1/rides/mine`, repository reads `listForDriver`, `listForRequester`, `listForPassenger`, frontend `api.getMyRides`, and server-backed `RideMinePage` sections for My Offers, My Requests, and My Trips / Participations. My Offers uses a mine-specific `RideMineOffer` summary with `participationCount`; it does not load or return full `participations[]`, passenger user IDs, passenger presentation objects, or meeting-point labels/coordinates. Dedicated manage-offer readback remains the authorized path for full participation details and Identity presentation enrichment.
- 2026-09-01 local proof: `npm -w @hooma/api run typecheck` passed after separating external `RideMineQuery` from parsed/defaulted `RideMineListQuery` and keeping `RideOfferForOwnerRecordList` distinct from contract DTO `RideOfferForOwnerList`.
- 2026-09-01 local proof: `npm -w @hooma/contracts run build`, `npm -w @hooma/frontend run typecheck`, `node --test tests/ride-mine-readback.test.mjs tests/frontend-router.test.mjs tests/requests-rides-shell.test.mjs`, `npx tsx --test tests/ride-service.test.ts`, `npm -w @hooma/frontend run build`, `npm -w @hooma/web run build`, `npm run architecture:check`, and `git diff --check` passed. Local Ride HTTP integration was not executed because `DATABASE_URL` is missing; permanent PostgreSQL integration coverage is in `tests/rides.http.integration.test.ts` for CI.
- 2026-09-01 overlap check before push: PR #206 `fix(play): make Open Matches match-owned and account-visible` is draft Play/Event work from the same base. It touches `apps/api/src/http/public-v1/router.ts`, Event/Play modules, `packages/database/prisma/schema.prisma`, Play migration, Event frontend, and Play tests; no Ride files or `apps/web/src/app/router/HoomaRouter.tsx` overlap with this `/rides/mine` branch.
- 2026-09-01 query-performance observation: existing `RideOffer(driverUserId, status, departureAt)`, `RideRequest(requesterUserId, status, createdAt)`, and `RideParticipation(passengerUserId, status, requestedAt)` indexes are useful by actor prefix for My Rides but do not perfectly match the unfiltered chronological ordering. No schema migration was added to PR #207; if production/EXPLAIN evidence later shows a material per-user history sort cost, add dedicated owner/time/id indexes in a separate bounded DB-performance change.
- 2026-09-01 tile-navigation correction branch `ride/ride-tile-request-context` keeps `/rides/matchday` and `/rides/anywhere` routes intact but changes only the Ride feature-card journey: `MATCHDAY RIDE` links to `/rides/request?context=MATCHDAY` and `ANYWHERE RIDE` links to `/rides/request?context=GENERAL`, relying on the existing request form `initialRideContext()` query parsing.
