        ...
        <section className="play-section" aria-labelledby="open-matches-title">
          ...
          {!accountLoading && !me ? (
            <div className="play-state panel">
              ...
              {signInHref ? (
                <a className="play-action play-action--commit" href={signInHref}>
                  Sign in
                </a>
              ) : null}
            </div>
          ...
          {!eventsError && me && events.length ? (
            <>
              ...
              {eventsNextCursor ? (
                <button
                  className="play-action play-action--secondary play-load-more"
                  type="button"
      ...
      {activeView === "players" ? (
        <section className="play-section" aria-labelledby="players-looking-title">
          ...
          {offerListing ? (
            <form
              ref={offerFormRef}
              className="play-sheet panel"
              onSubmit={sendOffer}
              tabIndex={-1}
            >
              <div className="play-sheet__person">
                <div className="play-sheet__avatar" aria-hidden="true">
                  {offerListing.presentation?.photoUrl ? (
                    <img src={offerListing.presentation.photoUrl} alt="" />
                  ) : (
                    <span>{(offerListing.presentation?.displayName ?? "P").slice(0, 1)}</span>
                  )}
                </div>
                <div>
                  <strong>{offerListing.presentation?.displayName ?? "Player"}</strong>
                  <small>
                    {offerListing.presentation?.username
                      ? `@${offerListing.presentation.username}`
                      : "Looking for a team"}
                  </small>
                </div>
              ...
              <div className="play-sheet__choices" role="listbox" aria-label="Teams">
                {recruitingTeams.map((team) => (
                  <button
                    ...
                    type="button"
                    role="option"
                    aria-selected={offerTeamId === team.id}
                    className={`play-sheet__choice${offerTeamId === team.id ? " is-selected" : ""}`}
                    disabled={offerLoading}
                    onClick={() => setOfferTeamId(team.id)}
                  >
              ...
              <button
                className="play-action play-action--commit play-action--block"
                type="submit"
          ...
          {inviteListing ? (
            ...
            >
              <div className="play-sheet__person">
                <div className="play-sheet__avatar" aria-hidden="true">
                  {inviteListing.presentation?.photoUrl ? (
                    <img src={inviteListing.presentation.photoUrl} alt="" />
                  ) : (
                    <span>{(inviteListing.presentation?.displayName ?? "P").slice(0, 1)}</span>
                  )}
                </div>
                <div>
                  <strong>{inviteListing.presentation?.displayName ?? "Player"}</strong>
                  <small>
                    {inviteListing.presentation?.username
                      ? `@${inviteListing.presentation.username}`
                      : "Looking for a game"}
                  </small>
                </div>
            ...
            <>
                ...
                {myListing ? (
                  <>
                    <span className="play-looking-badge">
                      <i aria-hidden="true" /> Live · Looking for{" "}
                      {lookingFor === "GAME" ? "a game" : "a team"}
                    </span>
                    <strong>Your listing is visible on Players</strong>
                    <div className="play-player-editor-actions">
                      <button
                        className="play-action play-action--secondary"
                        type="button"
                        onClick={() => setActiveView("players")}
                      >
                        Update
                  ...
                  <>
                    ...
                    <button
                      className="play-action play-action--primary"
                      type="button"
                      onClick={() => setActiveView("players")}
                    >
                      Publish that you want a game
                    </button>
                  </>
                )}
              </div>
              ...
              {managedEvents.length ? (
                <div className="play-mine-games">
                  {managedEvents.map((managedEvent) => (
                    <a
                      className="play-mine-game"
                      href={`/events/${managedEvent.id}`}
                      key={managedEvent.id}
                    >
                      <strong>{managedEvent.title}</strong>
