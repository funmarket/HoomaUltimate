...
function makePreset(
  ...
  return {
    id,
    label,
    slots: [{ position: "GK", x: 50, y: 91 }, ...lines.flatMap((line) => makeLine(line.y, line.positions))],
  };
}
...
function makeSlots(format: Format, presetId = defaultFormation(format).id): Slot[] {
  const preset = FORMATION_PRESETS[format].find((candidate) => candidate.id === presetId) ?? defaultFormation(format);
...
function formatLabel(format: Format): string {
  return FORMAT_OPTIONS.find((option) => option.value === format)?.label ?? format.replaceAll("_", " ");
}

function PlayerPortrait({ player, fallback }: { readonly player?: FormationRosterPlayer; readonly fallback: string }) {
          ...
          return (
            <section key={team} className="formation-team formation-team--game" style={teamStyle}>
              <div className="formation-team__heading">
                <h2>Team {team}</h2>
                <span>{FORMATION_PRESETS[format].find((preset) => preset.id === formationId)?.label}</span>
              ...
              <section className="formation-bench" aria-label={`Team ${team} bench`}>
                ...
                <p className="formation-bench__hint">
                  Tap a player on the pitch to change the assignment · Drag a bench player onto a pitch slot
                </p>
              </section>
            </section>
          );
