...
export function EventDetailPage({ eventId }: { readonly eventId: string }) {
  ...
  async function reloadEvent() {
    try { setEvent(await eventApi.publicDetail(eventId)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to load event"); }
  }
  ...
  async function join() {
    setActionPending(true); setError(""); setStatus("");
    try {
      ...
      await reloadEvent();
    } catch (reason) { setError(protectedError(reason, "Unable to join event")); }
  ...
  async function leave() {
    ...
    try {
      ...
      setRsvp(null); setStatus("RSVP cancelled."); await reloadEvent();
    } catch (reason) { setError(protectedError(reason, "Unable to leave event")); }
    finally { setActionPending(false); }
  }
    ...
    <div className="play-event-page">
      <section className="play-event-card">
        ...
        <div className="play-event-card__title-row">
          ...
          <div><h1>{event.title}</h1>{event.description ? <p>{event.description}</p> : null}</div>
        </div>
        ...
        {isWatch ? (
          <div className="play-event-card__facts">
            <div><span>Place</span><strong>{event.place?.name || "—"}</strong></div>
            <div><span>Houma</span><strong>{event.place?.houma || "—"}</strong></div>
            <div><span>City</span><strong>{event.place?.city || "—"}</strong></div>
            <div><span>Status</span><strong>{event.venueAuthority === "OFFICIAL_VENUE" ? "Official venue" : "Suggested by community"}</strong></div>
          </div>
        ) : (
          <div className="play-event-card__facts">
        ...
        {isWatch && event.place ? <a className="play-event-primary-action" href={`/places/${event.place.id}`}>View Place</a> : null}
        {rsvpLabel ? <div className="play-event-rsvp-state">{rsvpLabel}</div> : null}
        {rsvp === "ATTENDED" ? <div className="play-event-primary-action play-event-primary-action--static">Checked in</div>
          : rsvp === "WAITLISTED" || rsvp === "CONFIRMED" ? <button className="play-event-primary-action" type="button" disabled={actionPending} onClick={() => void leave()}>{actionPending ? "Updating…" : rsvp === "WAITLISTED" ? "Leave waitlist" : "Cancel RSVP"}</button>
          : eventOpen ? <button className="play-event-primary-action" type="button" disabled={actionPending || participationLoading} onClick={() => void join()}>{actionPending ? "Joining…" : isWatch ? "Join event" : "Join in one tap"}</button> : null}
      ...
      {!isWatch ? (
        <section className="play-matchday-hub" aria-labelledby="play-matchday-title">
          <div><p className="eyebrow">Matchday hub</p><h2 id="play-matchday-title">Event actions</h2></div>
