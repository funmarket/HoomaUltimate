...
export type FormationRosterPlayer = { userId: string; status: "CONFIRMED" | "ATTENDED"; presentation: { displayName: string; username: string; photoUrl: string | null } | null };
...
export function createEventApi(transport: HoomaTransport) {
  return {
    ...
    formationRoster: (id: string) => request<FormationRoster>(transport, `/api/v1/events/${encodeURIComponent(id)}/formation-roster`),
    ...
    cancelRsvp: (id: string) => request<{ cancelled: boolean; promotedUserId: string | null }>(transport, `/api/v1/events/${encodeURIComponent(id)}/rsvp`, { method: "DELETE" }),
    cancel: (id: string) => request(transport, `/api/v1/events/${encodeURIComponent(id)}/cancel`, { method: "POST" }),
    complete: (id: string) => request(transport, `/api/v1/events/${encodeURIComponent(id)}/complete`, { method: "POST" }),
    formations: (id: string) => request<FormationRecord[]>(transport, `/api/v1/events/${encodeURIComponent(id)}/formations`),
    createFormation: (id: string, input: EventFormationInput) => request<FormationRecord>(transport, `/api/v1/events/${encodeURIComponent(id)}/formations`, { method: "POST", body: JSON.stringify(input) }),
    checkIn: (id: string, latitude?: number, longitude?: number) => request(transport, `/api/v1/events/${encodeURIComponent(id)}/check-in`, { method: "POST", body: JSON.stringify({ latitude: latitude ?? null, longitude: longitude ?? null }) }),
    chat: (id: string) => request<EventChatRecord[]>(transport, `/api/v1/events/${encodeURIComponent(id)}/chat`),
    postChat: (id: string, body: string) => request<EventChatRecord>(transport, `/api/v1/events/${encodeURIComponent(id)}/chat/messages`, { method: "POST", body: JSON.stringify({ body }) })
  };
}
