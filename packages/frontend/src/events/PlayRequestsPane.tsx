...
export function PlayRequestsPane() {
  ...
  const [filters, setFilters] = useState<RequestsListQuery>({ surface: "PLAY" });
  ...
  useEffect(() => {
    let active = true;
    setError("");
    void Promise.all([requestsApi.taxonomy("PLAY"), api.identity.meOptional()])
      ...
      .catch((reason) => {
        if (!active) return;
        setError(protectedError(reason, "Unable to load Play Requests"));
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active) setError(protectedError(reason, "Unable to load Play Requests"));
      } finally {
  ...
  async function loadMore() {
    ...
    try {
      ...
      setError(protectedError(reason, "Unable to load more Play Requests"));
    } finally {
  ...
  return (
    <section className="play-section play-requests" aria-labelledby="play-requests-title">
      <div className="play-section-heading">
        <div>
          <p className="eyebrow">Requests</p>
          <h2 id="play-requests-title">What players need</h2>
        </div>
        <a className="play-action play-action--commit" href="/requests/new?surface=PLAY">
          Create Request
        </a>
      </div>
