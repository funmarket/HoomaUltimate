  ...
  return (
    <section className="place-page">
      <header className="place-page__header">
        ...
        <a className="place-primary-link" href="/places/new">Add a Place</a>
      </header>
    ...
    return (
      <section className="place-page">
        <div className="place-submitted panel">
          ...
          <p>The App Admin will review the Place. Once approved, it will appear in Places and can be used for Watch events.</p>
          <a className="place-primary-link" href="/watch">Back to Watch</a>
        </div>
      </section>
    );
  }
    ...
    <section className="place-page">
      <header className="place-page__header">
        <div>
          ...
          <p>Submit the business information once. App Admin approval makes it available to Watch.</p>
        </div>
      </header>
      <form className="panel place-add-form" onSubmit={(event) => void submit(event)}>
        <div className="place-form-grid">
          <label>Place name<input name="name" required minLength={2} /></label>
          <label>Category<input name="category" placeholder="Sports café, bar, restaurant…" /></label>
        </div>
        <label>Description<textarea name="description" rows={4} placeholder="Tell people about the Place" /></label>
        <label>Image URL<input name="imageUrl" type="url" placeholder="https://…" /></label>
        <label>Address<input name="address" required minLength={3} /></label>
        <div className="place-form-grid">
          <label>City<input name="city" /></label>
          <label>Houma<input name="houma" /></label>
        </div>
        <div className="place-form-grid">
...
export function PlaceDetailPage({ placeId }: { readonly placeId: string }) {
  ...
  useEffect(() => {
    void Promise.all([management.places.get(placeId), eventsApi.publicWatch()])
      ...
      .catch((reason) => setError(reason instanceof Error ? reason.message : "Unable to load Place"));
  ...
  async function claim(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      await management.places.claimOwnership(placeId, { evidence: String(data.get("evidence") ?? "") });
  ...
  if (!place) return error ? <p className="error">{error}</p> : <p className="status">Loading Place…</p>;

  return (
    <section className="place-detail-page">
      <a className="place-back-link" href="/places">← Places</a>
      ...
      <div className="place-info-grid">
        <article><span>Address</span><strong>{place.address}</strong></article>
        <article><span>Houma</span><strong>{place.houma || "—"}</strong></article>
        <article><span>City</span><strong>{place.city || "—"}</strong></article>
        <article><span>Contact</span><strong>{place.phone || place.email || "—"}</strong></article>
      </div>
      <section className="place-events-section">
        <div className="section-heading"><div><p className="eyebrow">WATCH</p><h2>Upcoming Watch events at this Place</h2></div><span>{events.length}</span></div>
        <div className="place-event-list">
          {events.map((event) => <a key={event.id} href={`/events/${event.id}`}><strong>{event.title}</strong><span>{new Date(event.startsAt).toLocaleString()} · {event._count.rsvps} going</span></a>)}
          {!events.length ? <p className="muted">No upcoming Watch events yet.</p> : null}
        </div>
      </section>
      <section className="place-claim-section">
        <button type="button" className="place-claim-toggle" onClick={() => setClaimOpen((value) => !value)}>Own/manage this place?</button>
        {claimOpen ? <form className="panel place-claim-form" onSubmit={(event) => void claim(event)}><label>Ownership or management evidence<textarea name="evidence" minLength={10} required /></label><button type="submit">Submit ownership claim</button></form> : null}
...
export function PitchPage() {
  ...
  useEffect(() => {
    void Promise.all([api.places.list(), api.capability.list("PITCH")])
      .then(([placeRows, capabilityRows]) => { setPlaces(placeRows); setItems(capabilityRows); })
      .catch((reason) => setError(reason instanceof Error ? reason.message : "Unable to load Pitch"));
  }, [api]);

  return (
    <section className="place-page">
      <header className="section-heading"><div><p className="eyebrow">PITCH</p><h1>Pitch</h1><p className="muted">Approved football pitches and bookable playing venues.</p></div></header>
      <div className="place-directory">
        {items.map((item) => <article className="panel place-card" key={item.id}><h2>{item.place.name}</h2><p>{locationLabel(item.place)}</p><p>{item.summary}</p></article>)}
      </div>
