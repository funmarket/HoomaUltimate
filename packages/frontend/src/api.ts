...
export function createHoomaApi(transport: HoomaTransport) {
  ...
  const whistles = {
    ...
    sendToEvent: (eventId: string, body: string) =>
      request<{ whistle: WhistleListItem; remainingToday: number; resetsAt: string }>(
        transport,
        `/api/v1/whistles/contexts/EVENT/${encodeURIComponent(eventId)}`,
        { method: "POST", body: JSON.stringify({ body }) },
  ...
  const teams = {
    ...
    createLineup: (teamId: string, input: TeamLineupInput) =>
      request(transport, `/api/v1/teams/${encodeURIComponent(teamId)}/lineups`, {
        method: "POST",
        body: JSON.stringify(input),
      }),
