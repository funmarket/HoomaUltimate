...
function athletesMemberListPath(id: string, cursor?: string): string {
  const params = new URLSearchParams({ limit: "50" });
  if (cursor) params.set("cursor", cursor);
  return `/api/v1/athletes/${encodeURIComponent(id)}/members?${params.toString()}`;
}

function athletesJoinRequestListPath(id: string, cursor?: string): string {
  const params = new URLSearchParams({ limit: "50" });
  if (cursor) params.set("cursor", cursor);
...
export function createAthletesApi(transport: HoomaTransport) {
  return {
    ...
    joinRequests: (id: string, cursor?: string) =>
      request<AthletesJoinRequestPage>(transport, athletesJoinRequestListPath(id, cursor)),
    ...
    members: (id: string, cursor?: string) =>
      request<AthletesMemberPage>(transport, athletesMemberListPath(id, cursor)),
