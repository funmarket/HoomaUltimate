...
test("Athletes private membership page queries are bounded", () => {
  ...
  assert.equal(athletesJoinRequestListQuerySchema.parse({ limit: "1" }).limit, 1);
...
test("Athletes private membership pages expose items and authoritative cursors", () => {
  const member = {
    userId: "runner-1",
    role: "MEMBER" as const,
    joinedAt: "2026-09-12T09:00:00.000Z",
  ...
  assert.deepEqual(
    ...
    { items: [member], nextCursor: "membership-1" },
  );
