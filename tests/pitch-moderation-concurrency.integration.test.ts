...
function rejectedWithCode(
  ...
  return result.status === "rejected" && result.reason instanceof AppError && result.reason.code === code;
}

test("Pitch moderation enforces one pending revision and one review winner under concurrency", async () => {
  ...
  try {
    ...
    assert.equal(
      reviews.filter((result) => rejectedWithCode(result, "PITCH_APPLICATION_REVIEW_NOT_PENDING"))
        .length,
      1,
    );
    ...
    assert.ok(reviewedRevision?.reviewedByUserId === adminA.id || reviewedRevision?.reviewedByUserId === adminB.id);
    ...
    await db.user.deleteMany({ where: { id: { in: [owner.id, adminA.id, adminB.id] } } });
  }
});
