...
test("creators and PLATFORM_ADMIN can edit/archive HOOMAs and Teams without destructive row deletion", async () => {
  ...
  try {
    ...
    await db.platformRoleAssignment.create({ data: { userId: admin.userId, role: "PLATFORM_ADMIN" } });
    ...
    const archivedTeam = await db.team.findUniqueOrThrow({ where: { id: team.id }, select: { status: true } });
    assert.equal(archivedTeam.status, "ARCHIVED");
    assert.equal(await db.teamPlayer.count({ where: { teamId: team.id, active: true, leftAt: null } }), 0);
    assert.equal(await db.teamResponsibilityAssignment.count({ where: { teamId: team.id, revokedAt: null } }), 0);
    ...
    const archivedCommunity = await db.community.findUniqueOrThrow({ where: { id: community.id }, select: { status: true } });
    assert.equal(archivedCommunity.status, "ARCHIVED");
    assert.equal(await db.communityMembership.count({ where: { communityId: community.id, leftAt: null } }), 0);
    ...
    assert.equal((await fetch(`${base}/api/public/v1/communities/${adminCommunity.id}`)).status, 404);
  } finally {
