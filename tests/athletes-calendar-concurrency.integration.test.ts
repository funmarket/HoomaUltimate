...
test(
  "Athletes Calendar lifecycle locking preserves Founder writes and RSVP concurrency safety",
  async (t) => {
    ...
    try {
      await t.test(
        "Founder Calendar create waits for Athletes archive and cannot write after archive commits",
        async () => {
          const community = await athletesService.create(founder!.id, {
          ...
          const archiveTransaction = athletesRepository.withCommunityLock(
            community.id,
            async (scoped) => {
              await new AthletesService(scoped, userLastSeenReader).archive(
                founder!.id,
                community.id,
              );
              archived.resolve();
          ...
          try {
            ...
            assert.equal(
              waiting,
              true,
              "Founder Calendar mutation must wait for the Athletes exclusive lifecycle lock",
            );
          } finally {
            release.resolve();
          ...
          await assert.rejects(competingCreate, expectCode("ATHLETES_NOT_FOUND"));
          assert.equal(
