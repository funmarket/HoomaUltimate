...
import type { UserLastSeenReader } from "../apps/api/src/modules/identity/application/user-last-seen.reader.js";
...
test("Athletes Calendar RSVP shared lifecycle guard preserves concurrency and destructive lifecycle safety", async (t) => {
  ...
  try {
    await t.test("two independent RSVP transactions can hold the shared guard concurrently", async () => {
      ...
      const firstRsvp = firstService.setRsvp(memberA!.id, community.id, entry.id, "GOING");
    ...
    await t.test("member removal blocks an in-flight RSVP until authority is rechecked", async () => {
    ...
    await t.test("Calendar cancellation blocks an in-flight RSVP and wins before the RSVP recheck", async () => {
    ...
    await t.test("community archive blocks an in-flight RSVP and prevents a post-archive write", async () => {
