...
import type { GamerArenaMatchList } from "@hooma/contracts/gamers";
...
test("global Gamer Arena exposes accepted active-game Match Cards with canonical photos", async () => {
  await cleanup();
  ...
  try {
    ...
    const body = (await response.json()) as GamerArenaMatchList;
