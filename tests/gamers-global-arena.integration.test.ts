...
test("global Gamer Arena is public, accepted-only, cross-game and uses canonical profile photos", async () => {
  await cleanup();
  ...
  try {
    ...
    const fixtureMatches = body.items.filter(
      (item) => item.challenger.presentation.username.startsWith(prefix),
    );
    assert.deepEqual(
