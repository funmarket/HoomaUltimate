...
test(
  "Coach Control Room consumes protected Team APIs rather than duplicating state",
  async () => {
...
test(
  "Team lineup separates Formation from match format and offers standard plus custom Formation",
  async () => {
    const page = await readFile(coachPagePath, "utf8");
...
test(
  "every Team player is backed by a HOOMA User without global user uniqueness",
  async () => {
    const contracts = await readFile("packages/contracts/src/index.ts", "utf8");
    const schema = await readFile("packages/database/prisma/schema.prisma", "utf8");
    assert.match(
