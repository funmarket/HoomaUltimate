...
test("standalone Requests uses one SPORT or COMMUNITY taxonomy flow", async () => {
  ...
  assert.match(filters, /taxonomy\.sports\.map/);
  assert.match(filters, /taxonomy\.community\.subcategories\.map/);
  assert.match(filters, /All categories/);
  assert.match(filters, /All needs/);
  assert.match(filters, /resetFilters/);
  assert.match(filters, /applyFilters/);
