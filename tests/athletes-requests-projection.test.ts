...
test("Athletes exposes canonical non-football Requests without owning Request persistence", async () => {
  const [page, pane, createPage, requestRepo, taxonomyService, filters] = await Promise.all([
    ...
    readFile("apps/api/src/modules/help-taxonomy/application/help-taxonomy.service.ts", "utf8"),
  ...
  assert.match(
    requestRepo,
    /input\.surface === "ATHLETES"[\s\S]*requestType: "SPORT"[\s\S]*NOT: \{ sport: "FOOTBALL" \}/,
  );
  assert.match(taxonomyService, /input\.surface === "ATHLETES"[\s\S]*row\.sport === "FOOTBALL"/);
});
