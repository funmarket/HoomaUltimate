...
function me(id: string, role: "FOUNDER" | "MEMBER") {
  return {
    ...
    athletesCommunities: [],
    moderation: { yellowCardCount: 0, isBanned: false, banExpiresAt: null, isReadOnly: false, readOnlyExpiresAt: null, isDisabled: false },
    teams: [],
  };
}

const otherResponse = {
  id: "response-1", requestId: "request-1", responderUserId: "user-2", message: "I have a spare pair.",
  status: "PENDING", createdAt: "2026-09-17T13:00:00.000Z", updatedAt: "2026-09-17T13:00:00.000Z",
  acceptedAt: null, declinedAt: null, withdrawnAt: null,
};
...
function installApiStub(options: {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.pathname === "/api/public/v1/requests/request-1") return json(options.request ?? baseRequest);
    ...
    if (url.pathname === "/api/v1/requests/request-1/responses" && method === "POST") {
      if (options.actionStatus && options.actionStatus >= 400) return json(options.actionBody, options.actionStatus);
...
async function renderDetail(options: Parameters<typeof installApiStub>[0]) {
  ...
  return { view, calls: stub.calls, fireEvent, waitFor,
    close: () => { stub.restore(); cleanup(); dom.window.close(); } };
}
...
test("a guest reads the public detail and is routed through auth to respond", async () => {
  ...
  try {
    ...
    assert.deepEqual(page.calls.filter((c) => c.includes("/requests/")), ["GET /api/public/v1/requests/request-1"]);
...
test("an eligible signed-in member sends one response and may withdraw it", async () => {
  ...
  try {
    ...
    page.fireEvent.change(page.view.getByLabelText(/your message|message/i), { target: { value: "I can help." } });
...
test("a manager sees player responses, can accept, fulfil, and never fakes success on conflict", async () => {
  ...
  try {
    ...
    await ok.waitFor(() => assert.ok(ok.calls.includes("POST /api/v1/requests/request-1/responses/response-1/accept")));
  ...
  try {
    ...
    await conflict.waitFor(() => assert.ok(conflict.view.container.querySelector(".request-error")));
