...
import {
  type OutboxHandler,
  OutboxRunner,
} from "../apps/worker/src/outbox/outbox.runner.js";

const databaseUrl = process.env.DATABASE_URL;
if (!databaseUrl) {
  throw new Error(
    "DATABASE_URL is required for Outbox worker integration tests",
  );
}
...
test(
  "Outbox runner leaves events untouched when no handlers are registered",
  async () => {
    await cleanup();
...
test(
  "Outbox runner delivers a registered topic exactly through its handler",
  async () => {
    await cleanup();
...
test(
  "Outbox runner releases a failed delivery with a bounded retry attempt",
  async () => {
    await cleanup();
