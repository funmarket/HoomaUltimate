...
test("Athletes member and join-request cursors traverse more than 1000 tied rows exactly once", async () => {
  ...
  const userIds = Array.from({ length: 1050 }, (_, index) =>
    `ath-page-user-${suffix}-${String(index).padStart(4, "0")}`,
  );
  ...
  try {
    ...
    console.log("Athletes member pagination plan:\n" + memberPlan.map((row) => row["QUERY PLAN"]).join("\n"));
    console.log(
