...
test("Event Whistle reuses canonical Event access and the global Whistle quota", async () => {
  ...
  try {
    ...
    assert.equal((await fetch(`${base}${eventPath}`, { headers: headers(outsider.cookie) })).status, 403);
    assert.equal((await sendWhistle(base, outsider.cookie, "EVENT", event.id, "not allowed")).status, 403);
    assert.equal((await fetch(`${base}${eventPath}`, { headers: headers(coach.cookie) })).status, 200);
    assert.equal((await fetch(`${base}${eventPath}`, { headers: headers(founder.cookie) })).status, 200);
    ...
    const confirmedPayload = (await confirmedSend.json()) as { whistle: { id: string; body: string }; remainingToday: number };
    ...
    assert.equal((await fetch(`${base}${eventPath}`, { headers: headers(confirmed.cookie) })).status, 403);

    const promoted = await fetch(`${base}/api/v1/events/${event.id}/rsvp`, { headers: headers(waitlisted.cookie) });
    assert.equal(promoted.status, 200);
    assert.equal(((await promoted.json()) as { rsvp: { status: string } | null }).rsvp?.status, "CONFIRMED");

    const disabledTeamContext = await fetch(`${base}${whistlePath("TEAM", "not-enabled")}`, { headers: headers(founder.cookie) });
