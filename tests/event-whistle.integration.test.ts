...
test("Event Whistle reuses canonical Event access and the global Whistle quota", async () => {
  ...
  try {
    ...
    const anonymous = await fetch(`${base}${eventPath}`);
    assert.equal(anonymous.status, 401);

    const outsiderList = await fetch(`${base}${eventPath}`, { headers: headers(outsider.cookie) });
    assert.equal(outsiderList.status, 403);
    const outsiderSend = await sendWhistle(base, outsider.cookie, "EVENT", event.id, "not allowed");
    assert.equal(outsiderSend.status, 403);

    const creatorList = await fetch(`${base}${eventPath}`, { headers: headers(coach.cookie) });
    assert.equal(creatorList.status, 200);
    const founderList = await fetch(`${base}${eventPath}`, { headers: headers(founder.cookie) });
