...
if (!databaseUrl) {
  throw new Error("DATABASE_URL is required for User Whistle integration tests");
}
if (!redisUrl) {
  throw new Error("REDIS_URL is required for User Whistle integration tests");
}
...
function directUrl(base: string, username: string): string {
  return `${base}/api/v1/whistles/users/${encodeURIComponent(username)}`;
}

function postDirect(base: string, cookie: string, username: string, body: string) {
  return fetch(directUrl(base, username), {
    method: "POST",
