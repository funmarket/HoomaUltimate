...
if (!databaseUrl) throw new Error("DATABASE_URL is required for Athletes Calendar integration tests");
...
test("Athletes private Calendar enforces membership, Founder mutation and community isolation", async () => {
  ...
  try {
    ...
    const editCancelled = await fetch(
      `${base}/api/v1/athletes/${firstId}/calendar/${created.id}`,
      {
        method: "PATCH",
        headers: headers(founder.cookie),
