...
test("Phase 1 identity slice uses the locked layered structure", async () => { await Promise.all(required.map((path) => access(path))); });
test("fresh schema keeps the identity foundation without premature donor-domain dumping", async () => {
  ...
  for (const model of ["User", "UserPresentation", "TelegramIdentity", "WebCredential", "WebSession", "PlatformRoleAssignment", "AuditLog", "OutboxEvent"]) assert.match(schema, new RegExp(`model ${model}\\b`));
  assert.doesNotMatch(schema, /model UltrasGroup\b|model GamerSquad\b|model Whistle\b/);
});
test("Whistle body persistence is not introduced by foundation schema", async () => { const schema = await readFile("packages/database/prisma/schema.prisma", "utf8"); assert.doesNotMatch(schema, /whistleBody|messageBody/i); });
