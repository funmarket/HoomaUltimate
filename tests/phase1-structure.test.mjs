...
test("fresh schema has identity foundation and no donor feature-table dump", async () => {
  const schema = await readFile("packages/database/prisma/schema.prisma", "utf8");
  for (const model of ["User", "UserPresentation", "TelegramIdentity", "WebCredential", "WebSession", "PlatformRoleAssignment", "AuditLog", "OutboxEvent"]) {
    assert.match(schema, new RegExp(`model ${model}\\b`));
  }
  assert.doesNotMatch(schema, /model Team\b|model UltrasGroup\b|model GamerSquad\b|model Whistle\b/);
});
