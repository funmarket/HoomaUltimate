...
const watchInput: EventCreateInput = {
  communityId: "community-1", type: "WATCH", title: "Derby night", startsAt: "2026-08-22T18:00:00.000Z",
  timezone: "Africa/Tunis", waitlistEnabled: true, entryFeeMinor: 0, currency: "TND"
};
const playInput: EventCreateInput = {
  communityId: "community-1", type: "PLAY", title: "Friday football", startsAt: "2026-08-22T18:00:00.000Z",
  timezone: "Africa/Tunis", waitlistEnabled: true, entryFeeMinor: 0, currency: "TND",
...
test("EventService rejects WATCH creation while Watch is frozen", async () => {
  let createCalled = false; let coachCheckCalled = false;
  const communities = { requireCoach: async () => { coachCheckCalled = true; } } as unknown as CommunityService;
  const service = new EventService(repositoryStub(() => { createCalled = true; }), communities);
  await assert.rejects(() => service.create("user-1", watchInput), (error: unknown) => error instanceof AppError && error.statusCode === 409 && error.code === "WATCH_NOT_ENABLED");
  assert.equal(createCalled, false); assert.equal(coachCheckCalled, false);
});

test("EventService still creates free PLAY events through the canonical repository", async () => {
  let createCalled = false; let coachCheckCalled = false;
  const communities = { requireCoach: async (communityId: string, userId: string) => { assert.equal(communityId, "community-1"); assert.equal(userId, "user-1"); coachCheckCalled = true; } } as unknown as CommunityService;
  const service = new EventService(repositoryStub(() => { createCalled = true; }), communities);
  await service.create("user-1", playInput);
  assert.equal(coachCheckCalled, true); assert.equal(createCalled, true);
});
...
test("EventService returns only the authenticated user's RSVP state", async () => {
  ...
  repository.getRsvp = async (eventId, userId) => { assert.equal(eventId, "event-1"); assert.equal(userId, "user-1"); return { status: "WAITLISTED" }; };
...
test("EventService rejects formation players outside the confirmed event roster", async () => {
  const repository = repositoryStub(() => {});
  repository.access = async () => ({ communityId: "community-1", createdByUserId: "user-1", status: "PUBLISHED", entryFeeMinor: 0n });
  repository.formationRoster = async () => [{ userId: "player-1", status: "CONFIRMED", presentation: null }];
  const service = new EventService(repository, {} as CommunityService);
  await assert.rejects(() => service.createFormation("user-1", "event-1", {
    name: "5v5", format: "FIVE_V_FIVE", published: true,
