...
test(
  "EventService rejects WATCH creation until the Watch vertical slice owns creation",
  async () => {
...
test(
  "EventService still creates free PLAY events through the canonical repository",
  async () => {
    let createCalled = false;
    let coachCheckCalled = false;
    const communities = {
...
test(
  "EventService rejects formation players outside the confirmed event roster",
  async () => {
    const repository = repositoryStub(() => {});
    repository.access = async () => ({
      communityId: "community-1",
