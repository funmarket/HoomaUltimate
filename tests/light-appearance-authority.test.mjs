...
const accountCss = new URL(
  "../apps/web/src/account/account.css",
  import.meta.url,
);
const profileCss = new URL(
  "../apps/web/src/profile/profile.css",
  import.meta.url,
);

test("Light exposes one semantic application palette", async () => {
...
test("legacy CSS no longer owns application colors", async () => {
  ...
  assert.match(
    css,
    /\.panel\s*\{[\s\S]*border:\s*1px solid var\(--app-line-strong\)/,
  );
  assert.match(
    css,
    /\.panel\s*\{[\s\S]*background:\s*var\(--app-surface\)/,
  );
  assert.doesNotMatch(css, /padding:\s*48px 24px/);
  assert.match(css, /padding:\s*0 24px 48px/);
});

test("account styles map to application semantics", async () => {
  ...
  assert.doesNotMatch(
    css,
    /:root\[data-theme="light"\]\s*\{[\s\S]*--hooma-bg/,
  );
...
test("top account chrome stays dark and owns safe-area spacing", async () => {
  ...
  assert.match(
    css,
    /\.hooma-topbar\s*\{[\s\S]*--hooma-chrome-bg:\s*#070808/,
  );
  assert.match(
    css,
    /\.hooma-topbar\s*\{[\s\S]*background:\s*var\(--hooma-chrome-bg\)/,
  );
  assert.match(
    css,
    /padding:\s*max\(8px, env\(safe-area-inset-top, 0px\)\) 24px 10px/,
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.hooma-account-menu\s*\{[\s\S]*color:\s*var\(--hooma-chrome-text\)/,
  );
});

test("Light profile copy stays readable beside a dark hero", async () => {
