...
test("locked bottom navigation and eight-card Home gateway cannot drift", async () => {
  ...
  const nav = indexSource.slice(indexSource.indexOf("export const PRIMARY_NAV_ITEMS"), indexSource.indexOf("export interface FoundationShellProps"));
  assert.deepEqual([...nav.matchAll(/label: "([^"]+)"/g)].map((match) => match[1]), ["Home", "Play", "Watch", "HOOMA", "Pitch"]);
  assert.deepEqual([...gatewaySource.matchAll(/label: "([^"]+)"/g)].map((match) => match[1]), ["HOOMA", "Teams", "Ultras", "Gamers", "Places", "Requests", "Ride", "FundMe"]);
  assert.deepEqual([...gatewaySource.matchAll(/href: "([^"]+)"/g)].map((match) => match[1]), ["/hooma", "/teams", "/ultras", "/gamers", "/places", "/requests", "/rides", "/fundme"]);
});
