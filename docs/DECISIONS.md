...
The product owner's later Requests correction further fixes the taxonomy root as exactly `SPORT | COMMUNITY`. SPORT reuses canonical `AthletesSport`; COMMUNITY is a parallel root and never requires a fake Sport. Corrected taxonomy must preserve root/subcategory/Need integrity, and SPORT must additionally preserve sport/subcategory integrity. User-entered Other text is per-Request `customNeed`, not a new global taxonomy row. Draft PR `#351` is the clean in-flight implementation of this correction and is not merged foundation truth until authorized and merged.
