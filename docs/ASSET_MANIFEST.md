...
6. All nine gateway cards use the same shared React component and the same 10:13 card geometry.
...
```text
HomeGatewayGrid
├── Row 1: HOOMA → /hooma | Teams → /teams | Ultras
├── Row 2: Spots → /places | Pitch → /pitch | Gamers → /gamers
...
```text
9 primary gateway cards in a phone-first 3 × 3 grid
↓
