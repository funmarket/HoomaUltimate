...
The Athletes Calendar is member-private Athletes-owned schedule data for one `AthletesCommunity`; it does not reuse Event, Play, Watch, Pitch or Gamers lifecycle state. Active same-community `FOUNDER`, `MODERATOR` and `MEMBER` memberships may list Calendar entries through bounded interval-overlap ranges of at most 45 days, while only the active same-community `FOUNDER` may create, edit or cancel entries. Calendar entries require `endsAt > startsAt`, store a valid IANA timezone, and new-entry UI defaults that timezone from `Intl.DateTimeFormat().resolvedOptions().timeZone`, using `UTC` only when the runtime cannot provide a valid IANA timezone; no GPS/location permission is required. Cancellation is one-way and idempotent, cancelled entries remain visible, and archived communities, outsiders and cross-community memberships cannot access active Calendar content. Calendar reads use ordinary active-member authorization without taking the Athletes lifecycle row lock. Founder Calendar create/edit/cancel mutations reuse the existing Athletes community exclusive `FOR UPDATE` lifecycle lock and recheck active Founder authority inside the same transaction. Active same-community members may RSVP `GOING`, `MAYBE`, or `NOT_GOING`; one response exists per User/Calendar entry and changing status updates that response. RSVP writes use a shared `FOR SHARE` Athletes lifecycle guard, recheck active membership and same-community/non-cancelled Calendar scope in the same transaction, and may proceed concurrently with other shared RSVP writers while still conflicting with exclusive archive, member-removal, and Calendar-cancellation lifecycle writes. RSVP rows remain durable after membership ends, but current Going/Maybe/Not-going aggregate counts include only current active same-community memberships. Cancelled entries reject new RSVP changes and use the same current-active-member aggregate semantics rather than pretending the retained rows are a cancellation-time snapshot. Athletes Calendar RSVP remains separate from generic Event RSVP/waitlist state.
