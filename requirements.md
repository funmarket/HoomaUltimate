...
Active Athletes displays only active, unrevoked, unexpired web sessions. Member rows show avatar, display name, `@username`, role, and text-formatted last-seen timestamp. If a user has no active sessions, they do not appear. Athletes uses Identity's batched `UserLastSeenReader` to avoid N+1 queries; no Redis presence, Telegram activity fallback, or online dots exist.
...
`WebSession.lastSeenAt` tracks the last time this session was actively resolved during an authenticated request. Touch operations throttle to once per 60 seconds per session to minimize database write load. A session is active only when it is unrevoked and unexpired. If a user has no active sessions, they are not considered active.
...
The intended Play communication mechanic is **Event Whistle Board through the shared Whistle engine**; Event Chat will be decommissioned in a later cleanup slice.

---

# 10. Whistle

Whistle is one shared transient signal engine.

Current Whistle behavior and TTL contract are defined in `docs/CANONICAL_MODEL.md`. Product visibility and quota reset use UTC calendar days. No Redis fallback to PostgreSQL body storage or log/audit/notification copying is allowed. Whistle body content never reaches logs or audit trails.

---

# 11. Roles and Authority

No scoped domain uses the word `Admin`.

Canonical global role is `PLATFORM_ADMIN`.

Scoped domain roles:

- Communities: `FOUNDER`, `COACH`, `MEMBER`;
- Teams: responsibility roles are `COACH`, `ASSISTANT`; capabilities are explicit and granular;
- Athletes: `FOUNDER`, `MODERATOR`, `MEMBER`;
- Events/Play: organizer authority with full-feature and delegated Team authority;
- Gamers: when implemented, uses its own role semantics;
- ULTRAS: when implemented, uses its own role semantics;
- Places: when implemented, uses its own role semantics;
- Pitch: when implemented, uses its own role semantics;
- Requests/Fundraising: when implemented, use their own semantics.

---

# 12. Deployment and testing

Production claims require evidence from the exact deployed commit.

A successful build alone does not prove:

- a complete user flow;
- auth/membership integration;
- durable persistence;
- async delivery;
- cross-service network behavior;
- storage/media lifecycle;
- user experience on actual mobile network;
- the feature working correctly rather than the test harness masking failure.

Verification pipeline:

- CI verification must pass: install, Prisma generate/validate/migrate deploy, architecture check, changed-file formatting, changed-source lint, typecheck, package build, unit tests, full build, integration tests, deploy preflight, security check, migration status;
- exact-commit production deployment;
- live runtime smoke testing where feasible, including authenticated real user-flow verification.

A task producing fewer than 7/10 points blocks the next step. A task producing 9+ requires live-runtime proof.
